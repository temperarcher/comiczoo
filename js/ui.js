/**
 * VERSION: 1.7.0 (Modale Edit/Nuovo Albo)
 */
export const UI = {
    // ... (HEADER, PUBLISHER_SECTION, SERIE_SECTION rimangono identici v1.6.0)
    HEADER: () => `
                ...
                <div class="flex flex-col md:flex-row w-full gap-4 items-center">
                    ...
                    <div class="flex gap-3 items-center shrink-0">
                        ...
                        <div class="flex bg-slate-900 p-1 rounded-full border border-slate-700 mr-1">
                            <button id="view-grid" class="view-btn active px-3 py-1.5 rounded-full transition-all text-slate-400 hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg></button>
                            <button id="view-list" class="view-btn px-3 py-1.5 rounded-full transition-all text-slate-400 hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" /></svg></button>
                        </div>
                        <div class="flex bg-slate-900 p-1 rounded-full border border-slate-700">
                            ...
                            <button data-filter="celo" class="filter-btn px-4 py-1.5 rounded-full text-xs font-bold uppercase transition-all">Celo</button>
                            <button data-filter="manca" class="filter-btn px-4 py-1.5 rounded-full text-xs font-bold uppercase transition-all">Manca</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    PUBLISHER_SECTION: (contentHtml) => `
        <section class="bg-slate-800/30 border-b border-slate-800 py-3">
            <div class="container mx-auto px-6">
                <div id="codici-bar" class="flex gap-3 overflow-x-auto pb-2 custom-scrollbar items-center">${contentHtml}</div>
            </div>
        </section>`,
    ...
    PUBLISHER_PILL: (data, isActive) => {
        ...
        return `<div id="codice-${data.id}" data-brand-id="${data.id}" class="codice-item codice-item-square flex-none w-14 h-14 md:w-16 md:h-16 bg-slate-800 border ${activeState} rounded-lg overflow-hidden flex items-center justify-center p-0 cursor-pointer transition-all duration-300"><img src="${data.immagine_url}" alt="${data.nome}" title="${data.nome}" class="w-full h-full object-cover"></div>`;
    },
    ...
    SERIE_SECTION: (contentHtml) => `
        <section class="bg-slate-900/50 border-b border-slate-800 py-4 overflow-hidden">
            <div class="container mx-auto px-6">
                <div id="serie-showcase" class="flex gap-4 overflow-x-auto pb-2 custom-scrollbar items-center">${contentHtml}</div>
            </div>
        </section>`,
    ...
    SERIE_ITEM: (serie) => `
        <div class="serie-showcase-item shrink-0 h-16 bg-slate-800 border border-slate-700 rounded-lg overflow-hidden cursor-pointer shadow-lg relative group" data-serie-id="${serie.id}">
            <div class="h-full"><img src="${serie.immagine_url}" title="${serie.nome}" class="h-full w-auto object-contain transition-transform group-hover:scale-105"></div>
            <button class="btn-edit-serie absolute top-1 right-1 bg-yellow-500 text-slate-900 p-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity text-[10px] z-10" data-edit-id="${serie.id}">✏️</button>
        </div>`,
    ...
    // MODALE VISUALIZZAZIONE (con aggiunta tasto Edit ✏️)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4 mt-4 text-center">
                    <div class="bg-slate-800/50 p-2 rounded border border-slate-700"><span class="block text-[8px] text-slate-500 uppercase">Condizione</span><span class="text-xs text-white">${issue.condizione}</span></div>
                    <div class="bg-slate-800/50 p-2 rounded border border-slate-700"><span class="block text-[8px] text-slate-500 uppercase">Valore</span><span class="text-xs text-green-400 font-bold">${issue.valore} €</span></div>
                </div>
            </div>
            <div>
    ...
    // MODALE FORM (Edit / Nuovo)
    ISSUE_FORM: (issue = {}) => `
        <div class="p-2">
            <h2 class="text-xl font-black text-white uppercase mb-6 tracking-tighter">${issue.id ? 'Modifica Albo' : 'Inserisci Nuovo Albo'}</h2>
            <form id="form-albo" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <input type="hidden" name="id" value="${issue.id || ''}">
                <div class="space-y-4">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Titolo Albo</label>
                        <input type="text" name="nome" value="${issue.nome || ''}" class="w-full bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-white focus:ring-1 focus:ring-yellow-500 outline-none" required>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Numero</label>
                            <input type="text" name="numero" value="${issue.numero || ''}" class="w-full bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-white outline-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Annata</label>
                            <input type="text" name="annata" value="${issue.annata || ''}" class="w-full bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-white outline-none">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">URL Immagine</label>
                        <input type="url" name="immagine_url" value="${issue.immagine_url || ''}" class="w-full bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-white outline-none">
                    </div>
                </div>
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="valore" value="${issue.valore || ''}" class="w-full bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-white outline-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Condizione</label>
                            <select name="condizione" class="w-full bg-slate-800 border border-slate-700 p-2.5 rounded-lg text-white outline-none">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Stato Possesso</label>
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer"><input type="radio" name="possesso" value="celo" ${issue.possesso !== 'manca' ? 'checked' : ''}> <span class="text-xs text-white">Celo</span></label>
                            <label class="flex items-center gap-2 cursor-pointer"><input type="radio" name="possesso" value="manca" ${issue.possesso === 'manca' ? 'checked' : ''}> <span class="text-xs text-white">Manca</span></label>
                        </div>
                    </div>
                </div>
                <div class="col-span-full pt-4 flex gap-3">
                    <button type="submit" class="flex-1 bg-yellow-500 hover:bg-yellow-400 text-slate-900 font-black py-3 rounded-lg uppercase text-xs tracking-widest transition-all">Salva Albo</button>
                    <button type="button" id="cancel-form" class="px-6 bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 rounded-lg uppercase text-xs">Annulla</button>
                </div>
            </form>
        </div>`,

    STORY_ROW: (storia, si, charsHtml) => `<div class="py-3 border-l-2 border-yellow-600 pl-4 mb-3 bg-slate-800/30 rounded-r-lg"><div class="flex justify-between items-center mb-2"><span class="text-[9px] text-yellow-600 font-bold uppercase">Pos. ${si.posizione}</span><h4 class="text-white font-bold flex-1 ml-3 text-sm">${storia.nome}</h4></div><div class="flex wrap gap-1.5">${charsHtml}</div></div>`,
    CHARACTER_TAG: (p) => `<div class="flex items-center gap-1 bg-slate-950 rounded-full pr-2 border border-slate-800"><img src="${p.immagine_url}" class="w-5 h-5 rounded-full object-cover"><span class="text-[9px] text-slate-400">${p.nome}</span></div>`
};