/**
 * VERSION: 8.9.0 (INTEGRALE - RIGID UI PROTECTION)
 * PROTEZIONE: Livelli 1, 2, 3 bloccati come da file originale.
 * AGGIORNAMENTO: Gestione Modale e Form con Codici Editori (Brand).
 */
export const UI = {
    ...
    // LIVELLO 3: SERIE (INVARIATO - LARGHEZZA VARIABILE)
    ...
    // LIVELLO 5: MODALE (Layout a due colonne con storie)
    MODAL_WRAPPER: () => `
    ...
    // LIVELLO 6: FORM (CON CODICI EDITORE)
        ...
        <div class="bg-slate-900 rounded-3xl border border-slate-700 shadow-2xl overflow-hidden">
            <form id="form-albo" class="p-8 grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-4 space-y-4">
                    <div class="aspect-[3/4] bg-slate-950 rounded-2xl border-2 border-dashed border-slate-800 overflow-hidden relative">
                        <img id="preview-cover" src="${issue.immagine_url || ''}" class="w-full h-full object-cover ${issue.immagine_url ? '' : 'hidden'}">
                    </div>
                    <input type="text" name="immagine_url" value="${issue.immagine_url || ''}" class="w-full bg-slate-800 border border-slate-700 p-3 rounded-xl text-sm text-white" placeholder="URL Cover">
                </div>
